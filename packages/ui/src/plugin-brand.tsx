...
function MastraMark(props: SVGProps<SVGSVGElement>) {
  return (
    <MarkSvg fillRule="evenodd" {...props}>
...
export function pluginBrand(plugin: string): PluginBrand {
  return PLUGIN_BRANDS[plugin as PluginId] ?? PLUGIN_BRANDS.mastra
}
...
export function PluginMark({
  ...
  const brand = pluginBrand(plugin)
  const Icon = ICONS[plugin as PluginId] ?? ICONS.mastra
  const box = size === 'md' ? 32 : 32
  ...
  return (
    <span
      ...
      role="img"
      className={cn(
        'inline-flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-md',
        className,
      )}
      style={{
    ...
    >
      <Icon
        ...
        height={glyph}
        fill={brand.fg}
        style={{ width: `${glyph}px`, height: `${glyph}px`, display: 'block', flexShrink: 0 }}
      />
    </span>
  )
}
