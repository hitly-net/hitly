...
export async function createProject(formData: FormData) {
  ...
  await database.insert(projects).values({
    ...
    plugin,
    credentials: { plugin, resumeSecret: generateResumeSecret() },
...
export async function deleteProjectApiKey(projectId: string, keyId: string) {
  ...
  await database.delete(projectApiKeys).where(eq(projectApiKeys.id, keyId))
  revalidatePath(`/projects/${projectId}`)
  revalidatePath(`/projects/${projectId}/config`)
}
...
export async function updateProjectMemberRole(projectId: string, membershipId: string, formData: FormData) {
  ...
  if (!isProjectRole(role)) throw new Error('Invalid role')
  const database = requireDb()
  await database.update(projectMemberships).set({ role }).where(eq(projectMemberships.id, membershipId))
  revalidatePath(`/projects/${projectId}/people`)
}
...
export async function removeProjectMember(projectId: string, membershipId: string) {
  await requireProjectAdmin(projectId)
  const database = requireDb()
  await database.delete(projectMemberships).where(eq(projectMemberships.id, membershipId))
  revalidatePath(`/projects/${projectId}/people`)
}
...
export async function deleteProjectRule(projectId: string, ruleId: string) {
  await requireProjectAdmin(projectId)
  const database = requireDb()
  await database.delete(projectRules).where(eq(projectRules.id, ruleId))
  revalidatePath(`/projects/${projectId}/rules`)
}
...
export async function toggleProjectChannel(projectId: string, channelId: string, enabled: boolean) {
  await requireProjectAdmin(projectId)
  const database = requireDb()
  await database.update(projectChannels).set({ enabled }).where(eq(projectChannels.id, channelId))
  revalidatePath(`/projects/${projectId}/channels`)
}

export async function retryWorkItem(approvalId: string) {
  const { user, role } = await getAppContext()
  const database = requireDb()
  const approvalRows = await database.select().from(approvals).where(eq(approvals.id, approvalId)).limit(1)
  ...
  const result = await retryResume({ approvalId })
...
export async function cancelWorkItem(approvalId: string) {
  ...
  const result = await cancelApproval({ approvalId, actorUserId: user.id })
  if ('error' in result && result.error) throw new Error(result.error)
  revalidateWorkItem(approvalId, approval.projectId)
}
...
export async function delegateWorkItem(approvalId: string, formData: FormData) {
  ...
  const memberRows = await database
    .select()
    .from(projectMemberships)
    .where(and(eq(projectMemberships.projectId, approval.projectId), eq(projectMemberships.userId, userId)))
    .limit(1)
  ...
  await database.update(approvals).set({ assignedUserId: userId }).where(eq(approvals.id, approval.id))
...
export async function decideWorkItem(approvalId: string, formData: FormData) {
  ...
  const result = await decideApproval({ approvalId, actorUserId: user.id, payload })
  if ('error' in result && result.error) throw new Error(result.error)
  revalidateWorkItem(approvalId, approval.projectId)
