...
After a successful resume, the work item stores the full origin response (`resumeData` sent to the caller, HTTP status, and body — for Mastra workflows, the completed run result). When evidence storage is enabled, the work item also displays a **View evidence receipt** link; see [Evidence Storage](/docs/envelope#evidence-storage) for how approval events are chained in the external store.
...
<DocShot
  ...
  caption="Decided work items with evidence storage show View evidence receipt"
/>
