...
The key must belong to `HITLY_PROJECT_ID`. Send `Idempotency-Key` when the origin may retry the suspend POST.
...
`GET /api/v1/approvals/:id` returns envelope, origin, `originFields`, decisions, and `canAct`.
