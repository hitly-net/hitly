...
If `loginctl show-user derek -p Linger` is not `yes`, user units stop after SSH logout. Enable with `sudo loginctl enable-linger derek`. Until then, a user cron keepalive is installed:
